...
Class Action {
	...
	function save_parcel(){
		...
		if(isset($save) && isset($ids)){
			...
			if (!empty($_POST['sender_email']) && !empty($_POST['recipient_email'])) {
		    
		    
		    $mail = new PHPMailer; 
    		$mail->isSMTP();                                     
            $mail->Host = 'mail.apaxlogistics.com';  
    	    $mail->SMTPAuth = true;
    	    $mail->Username = 'noreply@apaxlogistics.com';
    	    $mail->Password = 'r[yM=xSf}jEZ';                          
    	    $mail->SMTPSecure = 'ssl';                            
    	    $mail->Port = 465;                                    
    	    $mail->setFrom('info@apaxlogistics.com', 'Courier');
            $mail->addAddress($_POST['sender_email'],$_POST['recipient_email']);     
            $mail->addReplyTo('info@apaxlogistics.com');
            $mail->isHTML(true);                                  
            $mail->Subject = 'New Appointment Enquiry' ;
    	    $mailContent = 'This mail is for only <b>Testing</b> purpose';
	...
	function delete_parcel(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM parcels where traking_number = $tracking");
		if($delete){
			return 1;
		}
	}
	function update_parcel(){
		extract($_POST);
		$update = $this->db->query("UPDATE parcels set status= $status where traking_number = $id");
		$save = $this->db->query("INSERT INTO parcel_tracks set status= $status , parcel_id = $id");
		if($status == 10)
		{
			$update = $this->db->query('UPDATE parcels set status_field = "'.$other_status.'" where traking_number = "'.$id.'"');
				$save = $this->db->query('INSERT INTO parcel_tracks set status= "'.$status.'" , status_field = "'.$other_status.'", parcel_id = "'.$id.'"');
		}
		if($update && $save)
			return 1;  
	}
	function get_parcel_heistory(){
		extract($_POST);
		$data = array();
		$parcel = $this->db->query("SELECT * FROM parcels where traking_number = '$ref_no'");
		if($parcel->num_rows <=0){
			...
			$status_arr = array("Item Accepted by Courier","Collected","Shipped","In-Transit","Arrived At Destination","Out for Delivery","Ready to Pickup","Delivered","Picked-up","Unsuccessfull Delivery Attempt","Other");
			while($row = $history->fetch_assoc()){
				if($status_arr[$row['status']] == 'Other')
				{
					$status = ucwords($row['status_field']);
				}else{
